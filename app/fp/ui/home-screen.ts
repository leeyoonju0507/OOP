...
class HomeScreen implements IHomeScreen {
  ...
  mainUI = async (user: ILoginData) => {
    console.log(
      user.nickname + '님 환영합니다~^^  ====>  ' + 'UserType:' + user.userType + '   ' + '잔액:' + user.money + '원',
    );
    ...
    while (1) {
      ...
      switch (select) {
        case '1':
          ...
          const canRegister = await this.productService.registerProduct(user.email, title, price, content);
          if (!canRegister) {
