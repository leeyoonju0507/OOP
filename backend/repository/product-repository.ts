...
class ProductRepository implements IProductRepository {
  ...
  storeProduct = async (title: string, content: string, price: number, sellerEmail: string, IsSoldOut: boolean) => {
    await this.db.writeCSV('products.csv', `${title},${content},${price},${sellerEmail},${IsSoldOut}`);
    return true;
  };
